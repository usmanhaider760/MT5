...
public class Risk_Manager : I_Risk_Manager
{
    ...
    public async Task<(bool Approved, Signal_Reject_Reason Reason, string Detail)> Validate_Trade_Risk_Async(
        ...
        List<Position_BO> open_positions)
    {
        ...
        if (account.Is_Daily_Loss_Limit_Hit)
            return (false, Signal_Reject_Reason.Daily_Loss_Limit_Reached, "Daily loss limit already reached. No new trades today.");

        if (account.Is_Weekly_Loss_Limit_Hit)
            return (false, Signal_Reject_Reason.Weekly_Loss_Limit_Reached, "Weekly loss limit reached. No new trades this week.");

        if (account.Is_Drawdown_Limit_Hit)
            return (false, Signal_Reject_Reason.Drawdown_Limit_Reached, "Account drawdown circuit breaker active.");
        ...
        if (daily_loss_pct >= _settings.Max_Daily_Loss_Percent)
            return (false, Signal_Reject_Reason.Daily_Loss_Limit_Reached, $"Daily loss {daily_loss_pct:F2}% reached max {_settings.Max_Daily_Loss_Percent}%.");
        ...
        if (weekly_loss_pct >= _settings.Max_Weekly_Loss_Percent)
            return (false, Signal_Reject_Reason.Weekly_Loss_Limit_Reached, $"Weekly loss {weekly_loss_pct:F2}% reached max {_settings.Max_Weekly_Loss_Percent}%.");

        if (account.Drawdown_From_Peak >= _settings.Account_Drawdown_Circuit_Breaker_Percent)
            return (false, Signal_Reject_Reason.Drawdown_Limit_Reached, $"Drawdown {account.Drawdown_From_Peak:F2}% hit circuit breaker {_settings.Account_Drawdown_Circuit_Breaker_Percent}%.");

        if (account.Consecutive_Losses >= _settings.Max_Consecutive_Losses_Before_Pause)
            return (false, Signal_Reject_Reason.Consecutive_Loss_Pause, $"Consecutive losses ({account.Consecutive_Losses}) hit pause threshold.");

        if (open_positions.Count >= _settings.Max_Open_Trades)
            return (false, Signal_Reject_Reason.Max_Open_Trades_Reached, $"Max open trades ({_settings.Max_Open_Trades}) already reached.");
        ...
        if (is_gold)
        {
            ...
            if (gold_count >= _settings.Max_Gold_Trades)
                return (false, Signal_Reject_Reason.Max_Gold_Trades_Reached, "Max Gold trades already open.");
        }

        // Correlation check: same-symbol or same-USD-direction exposure
        var correlation_result = Check_Correlation(signal, open_positions);
        if (!correlation_result.ok)
            return (false, correlation_result.reason, correlation_result.detail);
        ...
        if (signal.Reward_Risk_Ratio < min_rr)
            return (false, Signal_Reject_Reason.Reward_Risk_Insufficient, $"R:R {signal.Reward_Risk_Ratio:F2} is below minimum {min_rr:F2} for {signal.Strategy}.");

        await Task.CompletedTask;
        return (true, Signal_Reject_Reason.None, string.Empty);
    }

    public decimal Calculate_Lot_Size(decimal account_equity, decimal risk_percent, decimal stop_loss_pips, decimal pip_value_per_lot, decimal lot_step = 0.01m, decimal max_lot = 5.0m)
    {
        ...
        floored = Math.Min(floored, max_lot);
    ...
    private static (bool ok, Signal_Reject_Reason reason, string detail) Check_Correlation(Trade_Signal_BO signal, List<Position_BO> open_positions)
    {
        // Check if same currency pair is already open
        var same_symbol = open_positions.Any(p => p.Symbol_Name == signal.Symbol_Name);
        if (same_symbol)
            return (false, Signal_Reject_Reason.Same_Symbol_Already_Open, $"{signal.Symbol_Name} already has an open position.");
        ...
        if (correlated_count >= Trading_Constants.Max_Active_Forex_Symbols)
            return (false, Signal_Reject_Reason.Correlation_Exposure_Exceeded, $"USD correlation limit reached ({correlated_count} correlated positions open).");
