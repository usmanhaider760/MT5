namespace MT5TradingBot.Core
{
    ...
    public static class LotCalculator
    {
            ...
            double accountLeverage = 200)
        {
            ...
            string sym = PipCalculator.NormalizeSymbol(symbol);

            double pipSize = GetPipSize(sym);
            double pipDistance = PipCalculator.DistanceInPips(entryPrice, stopLoss, pipSize);
        ...
        public static double GetPipSize(string sym) => PipCalculator.GetPipSize(sym);
        ...
        {
            string s = PipCalculator.NormalizeSymbol(sym);
            double pipSize = GetPipSize(s);
            double pips = PipCalculator.DistanceInPips(entry, sl, pipSize);
